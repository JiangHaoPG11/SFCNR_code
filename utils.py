...
class zeroshot_news_simple_tower(torch.nn.Module):
    def __init__(self, args):
        ...
        self.news_id_encoder_layer1 = nn.Linear(self.embedding_origin_dim, self.args.embedding_dim)
        self.news_id_encoder_layer2 = nn.Linear(self.args.embedding_dim + self.embedding_origin_dim,  self.embedding_origin_dim)

        self.news_content_encoder_layer1 = nn.Linear(self.embedding_origin_dim, self.args.embedding_dim)
        self.news_content_encoder_layer2 = nn.Linear(self.args.embedding_dim + self.embedding_origin_dim,  self.embedding_origin_dim)

        self.news_id_decoder_layer1 = nn.Linear(self.embedding_origin_dim, self.args.embedding_dim)
        self.news_id_decoder_layer2 = nn.Linear(self.args.embedding_dim + self.embedding_origin_dim,  self.embedding_origin_dim)
    ...
    def news_id_encoder(self, x):
        ...
        qs = self.tanh(self.news_id_encoder_layer2(torch.cat([xd, x1], dim = -1)))
        return qs
    ...
    def news_id_decoder(self, x):
        ...
        x1 = self.leaky_relu(self.news_id_decoder_layer1(xd))
        x1 = F.dropout(x1, p=self.dropout_prob, training=self.training)
        v_s = self.tanh(self.news_id_decoder_layer2(torch.cat([xd, x1], dim = -1)))
        return v_s[:self.args.batch_size], v_s[self.args.batch_size:]
    ...
    def forward(self, news_content_rep, news_id_rep, news_feature_list, news_type_index):
        ...
        #                      torch.flatten(id_byatt, 0, 1),
        #                      torch.flatten(news_id_rep, 0, 1)).view(self.args.batch_size, self.args.sample_size, -1)
        return loss_zeroshot, feature_byid
...
class zeroshot_user_simple_tower(torch.nn.Module):
    ...
    def user_id_encoder(self, x):
        ...
        x1 = self.leaky_relu(self.user_id_encoder_layer1(xd))
        x1 = F.dropout(x1, p=self.dropout_prob, training=self.training)
        qs = self.tanh(self.user_id_encoder_layer2(torch.cat([xd, x1], dim=-1)))
        return qs
    ...
    def user_id_decoder(self, x):
        ...
        x1 = self.leaky_relu(self.user_id_decoder_layer1(xd))
        x1 = F.dropout(x1, p=self.dropout_prob, training=self.training)
        v_s = self.tanh(self.user_id_decoder_layer2(torch.cat([xd, x1], dim=-1)))
        return v_s[:self.args.batch_size], v_s[self.args.batch_size:]
    ...
    def forward(self, user_beh_rep, user_id_rep, user_type_index):
        ...
        # output = torch.where(user_type_index.unsqueeze(1) == 0, beh_byid, user_beh_rep).view(self.args.batch_size, -1)
        return loss_zeroshot, beh_byid, La, Lc, Ld
...
class zeroshot_user_contrastive_tower(torch.nn.Module):
    ...
    def user_att_transfer(self, x):
        # xd = F.dropout(x, p=self.dropout_prob, training=self.training)
        x1 = self.user_att_transfer_layer1(x)
        # x1 = F.dropout(x1, p=self.dropout_prob, training=self.training)
    ...
    def forward(self, user_beh_rep, user_att_rep, user_type_index):
        ...
        # loss_zeroshot =  torch.mean( - torch.log( torch.exp(Lp / 0.2) / (torch.exp(Lp/ 0.2) + torch.exp(Ln/ 0.2))))

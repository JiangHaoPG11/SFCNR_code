...
class news_encoder(torch.nn.Module):
    def __init__(self, args):
        ...
        self.attention = Additiveention(self.args.query_vector_dim, self.args.attention_dim * self.args.attention_heads)
...
class user_encoder(torch.nn.Module):
    def __init__(self, args):
        ...
        self.attention = Additiveention(self.args.query_vector_dim, self.multi_dim)
    ...
    def forward(self, user_interest_index):
        ...
        # 融合
        user_rep = torch.tanh(self.attention(user_interest_embedding.unsqueeze(0)))
...
class news_encoder(torch.nn.Module):
    def __init__(self, word_dim, num_filters, window_sizes, query_vector_dim):
        ...
        self.newsention = QueryAttention(query_vector_dim, num_filters)
    ...
    def forward(self, word_embedding, user_embedding):
        ...
        news_rep = torch.tanh(self.newsention(user_embedding, word_embedding))
...
class user_encoder(torch.nn.Module):
    def __init__(self, word_dim, num_filters, window_sizes, query_vector_dim):
        super(user_encoder, self).__init__()
        self.userention = QueryAttention(query_vector_dim, num_filters)
        self.dropout_prob = 0.3

    def forward(self, clicked_news_rep, user_embeding):
        user_rep = torch.tanh(self.userention(user_embeding.unsqueeze(0).unsqueeze(0), clicked_news_rep.unsqueeze(0)))
        user_rep = F.dropout(user_rep, p=self.dropout_prob, training=self.training)
        return user_rep
...
class Zeroshot_Contrastive_NPA(torch.nn.Module):
    def __init__(self, args, entity_embedding, relation_embedding, news_entity_dict, entity_adj, relation_adj,
        ...
        super(Zeroshot_Contrastive_NPA, self).__init__()
        self.args = args
        self.device = device
        ...
        # 属性编码器
        self.news_encoder = news_encoder(args)
        self.user_encoder = user_encoder(args)
    ...
    def get_user_news_rep(self, candidate_news_index, user_index, user_clicked_news_index):
        # 单词嵌入
        candidate_news_word_embedding = self.word_embedding[self.news_title_word_dict[candidate_news_index]].to(self.device)
        ...
        for i in range(self.args.sample_size):
            ...
            news_rep_one = self.news_encoder(news_index, news_category_index, news_word_embedding_one)
            if i == 0:
        ...
        for i in range(self.args.batch_size):
            ...
            # 用户表征
            user_rep_one = self.user_encoder(clicked_news_category_index).unsqueeze(0)
    ...
    def forward(self, candidate_newsindex, user_index, user_clicked_newsindex, user_type_index, news_type_index):
        ...
        # zeroshot学习(Att)
        news_rep, user_rep = self.get_user_news_rep(candidate_newsindex, user_index, user_clicked_newsindex)
        loss_zeroshot_user, user_rep_update, Lp, Ln = self.zeroshot_user_tower(user_rep, user_rep.squeeze(), user_type_index.to(self.device))
        loss_zeroshot_news, news_rep_update = self.zeroshot_news_tower(news_rep, news_rep, news_feature_list, news_type_index.to(self.device))
        ...
        # replace
        # user = torch.where(user_type_index.to(self.device).unsqueeze(1) == 0, user_rep.squeeze(),
        ...
        return rec_score, rec_score, loss_zeroshot_news, loss_zeroshot_user, Lp, Ln
    ...
    def test(self, candidate_newsindex, user_index, user_clicked_newsindex, user_type_index, news_type_index ):
        ...
        # zeroshot学习(Att)
        news_rep, user_rep = self.get_user_news_rep(candidate_newsindex, user_index, user_clicked_newsindex)
        _, user_rep_update, _, _ = self.zeroshot_user_tower(user_rep, user_rep.squeeze(), user_type_index.to(self.device))
        _, news_rep_update = self.zeroshot_news_tower(news_rep, news_rep, news_feature_list, news_type_index.to(self.device))
        ...
        # replace
        # user = torch.where(user_type_index.to(self.device).unsqueeze(1) == 0, user_rep.squeeze(), user_rep.squeeze()).view(self.args.batch_size, 1, -1)
        # news = torch.where(news_type_index.to(self.device).unsqueeze(-1) == 0, newsId_rep, news_rep)
        # score = self.predict_layer(user.repeat(1, news.shape[1], 1), news)
        news = torch.where(news_type_index.to(self.device).unsqueeze(-1) == 0, news_rep_update, news_rep)
        user = torch.where(user_type_index.to(self.device).unsqueeze(1) == 0, user_rep_update.squeeze(), user_rep.squeeze())
